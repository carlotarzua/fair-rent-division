...
st.caption(
    "An optimization-based calculator for assigning rooms and rent prices so that no roommate envies another."
)

with st.expander("What does this app do?", expanded=True):
    st.markdown(
        """
        This app solves a fair rent division problem. Each roommate enters how much
        they value each room. The algorithm chooses:
...
n = st.sidebar.slider(
    ...
    max_value=6,
    value=3,
    help="The current implementation handles equal numbers of roommates and rooms.",
)
...
allow_negative_prices = st.sidebar.checkbox(
    ...
    value=False,
    help=(
        "Some mathematical rent-division models allow a very undesirable room to have a negative price. "
        "For most real leases, keep this off so every room price is at least zero."
    ),
)

st.sidebar.markdown("---")
st.sidebar.markdown(
    "**Portfolio note:** this project demonstrates linear programming, fair division, Python, SciPy, pandas, and Streamlit."
)
...
st.write(
    ...
    "For the cleanest rent-division interpretation, each person's column should add up to the total rent."
)
...
with st.expander("Check valuation totals"):
    st.dataframe(sum_table, use_container_width=True, hide_index=True)
...
if st.button("Compute envy-free rent division", type="primary"):
    try:
        # The app table is rooms x people; the algorithm expects people x rooms.
        ...
        st.metric("Minimum assigned utility", round(float(result.min_utility), 2))
